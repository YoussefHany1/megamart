...
const EssentialCard = ({ item }: { item: EssentialItem }) => {
  ...
  return (
    <div className="item lg:w-1/6 md:w-1/4 w-1/2 mb-5" key={id}>
      ...
      >
        <div className="photo grid place-content-center text-center rounded-xl overflow-hidden h-40 outline-2 outline-white bg-(--background1) transition-all duration-200 hover:outline-(--primary)">
          <Image
            src={imageSrc}
            alt={imageAlt}
            width={200}
            height={200}
            className="max-w-full h-auto min-w-25"
            loading="lazy"
            quality={75}
        ...
        <div className="text text-center mt-3">
          <span className="text-(--secondary) text-nowrap block">{name}</span>
...
const SectionHeader = () => (
  <header className="header text-(--secondary) flex justify-between font-bold items-center">
  ...
  return (
    <section className="daily my-5 py-3" aria-labelledby="essentials-heading">
      ...
      <div
        ...
        role="list"
      >
        {ESSENTIALS.map((item: EssentialItem) => (
