...
import { ProductDetailsProps, ProductSpecification, ProductDetail } from "../types";

const TechnicalDetails = ({ specifications, details }: ProductDetailsProps) => {
  const hasData =
  ...
  return (
    <div className="technical-details mb-12 lg:mb-0">
      ...
      <table className="w-full">
        <tbody>
          {specifications?.map((item: ProductSpecification, index: number) => (
            <tr
              key={`spec-${index}`}
          ...
          {details?.map((item: ProductDetail, index: number) => (
            <tr
              key={`detail-${index}`}
              className={index % 2 === 1 ? "bg-gray-100" : ""}
            >
